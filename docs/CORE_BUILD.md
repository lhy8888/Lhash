...
pulling the Windows MFC release line back into the core build. On Apple arm64,
BLAKE3 also exercises the NEON implementation path while still checking the
same canonical digest values. The additional provider algorithms remain built
in the core target, but they are not part of the M2 smoke contract.
