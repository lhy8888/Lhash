...
public sealed class HashContractUnitTests
{
    ...
    public void HashEngine_StartsFromHashRequest_AndEmitsProgressEvents()
    {
        ...
        Assert.Contains("bool wasStopped = ProcessOpenedFileHashing(executionContext, request, result, fileIndex, isSizeCaled, fSizes, executionState", fileRunner, StringComparison.Ordinal);
        ...
        Assert.Contains("UpdateDigestContextsParallel(digestUpdateRequest", digestQueue, StringComparison.Ordinal);
        Assert.Contains("ProcessOpenedFileHashingSinglePass(executionContext, digestUpdateRequest, fsize, isSizeCaled, executionState);", digestPipeline, StringComparison.Ordinal);
