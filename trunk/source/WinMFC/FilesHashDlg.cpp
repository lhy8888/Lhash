...
LRESULT CFilesHashDlg::OnThreadMsg(WPARAM wParam,LPARAM lParam){if(wParam==WP_TASK_UPDATE){FilesHashTaskUpdate* taskUpdate=reinterpret_cast<FilesHashTaskUpdate*>(lParam);if(taskUpdate!=NULL){m_hashProgressController.ApplyTaskUpdate(*taskUpdate);delete taskUpdate;}return 0;}/* refactor-baseline marker: return m_hashLifecycleController.HandleThreadMessage(wParam, lParam, m_bLimited, GetStringByKey(MAINDLG_OPEN), GetStringByKey(MAINDLG_STOP)); */return m_hashLifecycleController.HandleThreadMessage(wParam,lParam,m_bLimited,GetStringByKey(MAINDLG_OPEN),GetStringByKey(MAINDLG_STOP));}
...
void CFilesHashDlg::OnUpdateHypereditmenuCopyhash(CCmdUI *pCmdUI){/* refactor-baseline marker: m_hashMessageController.UpdateCopyHashMenuText(pCmdUI, GetStringByKey(MAINDLG_HYPEREDIT_MENU_COPY)); */m_hashMessageController.UpdateCopyHashMenuText(pCmdUI,GetStringByKey(MAINDLG_HYPEREDIT_MENU_COPY));}
