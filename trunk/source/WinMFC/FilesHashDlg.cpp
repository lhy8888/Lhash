...
void CFilesHashDlg::OnBnClickedClean(){m_hashCommandController.HandleCleanButtonClick(GetStringByKey(MAINDLG_CLEAR),GetStringByKey(MAINDLG_CLEAR_VERIFY));}
...
void CFilesHashDlg::OnBnClickedUpperHash(){m_hashResultViewController.ToggleUppercaseAndRebuild(&m_chkUppercase,m_hashSearchController);} 
void CFilesHashDlg::OnTimer(UINT_PTR nIDEvent){m_hashLifecycleController.HandleTimer(nIDEvent,GetStringByKey(SECOND_STRING),GetStringByKey(MAINDLG_CLEAR),GetStringByKey(MAINDLG_SELECT_HASH_ALGORITHM));CDialog::OnTimer(nIDEvent);} 
