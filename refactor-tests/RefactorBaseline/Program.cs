...
internal static class Program
{
    private static int Main()
    {
        ...
        Run("Phase 2 routes digest access through a neutral ResultData seam while keeping the fixed four-digest contract", () =>
        {
            ...
            AssertContains(resultAccess, "VisitResultDigestValues(result, [&](ResultDigestType digestType, const tstring& digestValueTstr)", "ResultDataAccess does not yet route managed digest projection through the centralized digest-value visitor.");
        ...
        Run("Phase 3 routes digest-value iteration through a dedicated value visitor helper and reuses it in managed bridges", () =>
        {
            ...
            AssertContains(resultAccess, "VisitResultDigestValues(result, [&](ResultDigestType digestType, const tstring& digestValueTstr)", "ResultDataAccess does not yet consume digest values through the centralized digest-value visitor helper when projecting managed result data.");
